...
import { CheckCircle2, Info, BookmarkPlus, BookmarkCheck, ShieldCheck, FileText, ChevronDown, ChevronUp, Clock, ExternalLink, Award } from 'lucide-react';
const SchemeCard = ({
  scheme,
  isSaved,
  toggleSave
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-3 mt-2">
                ...
                <span className="text-sm font-medium text-slate-600 bg-slate-100 px-2.5 py-1 rounded-full flex items-center gap-1">
                  <span className="text-brand-600">{scheme.eligibilityPercentage}%</span>{t("SchemeCard.match")}</span>
                <span className="text-sm font-medium text-slate-600 bg-slate-100 px-2.5 py-1 rounded-full flex items-center gap-1">
        ...
        opacity: 0
            ...
            <div className="p-6 bg-slate-50">
              <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 flex gap-3 mb-6">
                ...
                <div>
                  <h4 className="text-sm font-bold text-emerald-900 uppercase tracking-wider mb-1">{t("SchemeCard.eligibility_reasoning")}</h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                  <h4 className="text-sm font-bold text-slate-900 mb-3 flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-brand-500" />{t("SchemeCard.application_tips")}</h4>
                  <ul className="space-y-2">
                    {scheme.applicationTips?.map((b, i) => <li key={i} className="text-sm text-slate-600 flex items-start gap-2">
                ...
                <div>
                  <h4 className="text-sm font-bold text-slate-900 mb-3 flex items-center gap-2">
                    <FileText className="w-4 h-4 text-brand-500" />{t("SchemeCard.required_documents")}</h4>
                  <ul className="space-y-2">
                    {scheme.requiredDocuments?.map((d, i) => <li key={i} className="text-sm text-slate-600 flex items-start gap-2">
                        <div className="w-1.5 h-1.5 rounded-full bg-slate-300 mt-1.5 flex-shrink-0"></div>
                        {d}
                      </li>)}
                  </ul>
              ...
              {scheme.importantNotes && <div className="bg-amber-50 rounded-lg p-4 text-sm text-amber-800 flex items-start gap-3 mb-6">
                  <Info className="w-5 h-5 mt-0.5 flex-shrink-0" />
                  <div>
                    <h4 className="font-semibold mb-1">{t("SchemeCard.important_notes")}</h4>
                    <p>{scheme.importantNotes}</p>
                  </div>
                </div>}
              
              {scheme.suggestedNextSteps && <div className="mb-6">
                  <h4 className="text-sm font-bold text-slate-900 mb-2">{t("SchemeCard.suggested_next_steps")}</h4>
                  <p className="text-sm text-slate-600">{scheme.suggestedNextSteps}</p>
                </div>}

              <div className="flex gap-4">
                <button className="flex-1 bg-brand-600 hover:bg-brand-700 text-white py-3 rounded-xl font-medium shadow-sm flex justify-center items-center gap-2 transition-colors">{t("SchemeCard.apply_now")}<ExternalLink className="w-4 h-4" />
                </button>
              </div>
            </div>
          </motion.div>}
