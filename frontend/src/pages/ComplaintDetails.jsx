...
const ComplaintDetails = () => {
  ...
  if (!complaint) {
    return <div className="text-center py-20 text-slate-500">Complaint not found.</div>;
  }
      ...
      <div id="report-content" className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border ${
                  complaint.priority === 'Critical' ? 'bg-red-100 text-red-700 border-red-200' :
                  complaint.priority === 'High' ? 'bg-orange-100 text-orange-700 border-orange-200' :
        ...
        <div className="p-8 sm:p-10 grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {complaint.imageUrl && (
              <section>
                <h3 className="text-lg font-bold text-slate-900 mb-3">Evidence Photo</h3>
                <div className="rounded-2xl overflow-hidden border border-slate-200">
                  <img src={`http://localhost:5000${complaint.imageUrl}`} alt="Evidence" className="w-full h-auto object-cover max-h-96" />
                </div>
              </section>
            )}
            ...
            {/* Timeline */}
            <section>
              <h3 className="text-lg font-bold text-slate-900 mb-6">Resolution Timeline</h3>
              <div className="relative border-l-2 border-slate-200 ml-3 space-y-8">
                {complaint.timeline.map((event, index) => (
                  <div key={index} className="relative pl-8">
                    <div className={`absolute -left-3 top-1 w-6 h-6 rounded-full border-4 border-white ${
                      index === complaint.timeline.length - 1 ? 'bg-brand-500' : 'bg-slate-300'
                    }`}></div>
             ...
             <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl p-6 border border-indigo-100 shadow-sm">
                ...
                <div className="space-y-4">
                  <div className="bg-white/60 p-3 rounded-xl border border-white">
                    <p className="text-xs font-semibold text-indigo-500 uppercase tracking-wider mb-1">Assigned Department</p>
                  ...
                  <div className="bg-white/60 p-3 rounded-xl border border-white">
                    <p className="text-xs font-semibold text-indigo-500 uppercase tracking-wider mb-1">Estimated Resolution</p>
                    <p className="text-sm font-bold text-slate-900">48 - 72 Hours</p>
                  </div>
                  <div className="bg-white/60 p-3 rounded-xl border border-white">
                    <p className="text-xs font-semibold text-indigo-500 uppercase tracking-wider mb-1">Risk Level</p>
