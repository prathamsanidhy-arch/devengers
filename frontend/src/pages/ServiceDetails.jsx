...
import { 
  ArrowLeft, ExternalLink, Bookmark, Shield, CreditCard, Zap, 
  Home, Droplets, Car, HeartPulse, Scale, FileText, CheckCircle, 
  MapPin, Clock, FileBadge, Scroll, AlertCircle, Bot
} from 'lucide-react';
...
const ServiceDetails = () => {
  ...
  useEffect(() => {
    ...
    if (service) {
      ...
      if (Object.keys(savedProgress).length === 0) {
        const initial = {};
        service.documents.forEach(doc => { initial[doc.id] = false; });
  ...
  if (!service) {
    return (
      <div className="py-8 px-4 max-w-7xl mx-auto flex flex-col items-center justify-center min-h-[60vh]">
  ...
  const toggleChecklist = (docId) => {
    const newChecklist = { ...checklist, [docId]: !checklist[docId] };
  ...
  const fetchAiGuidance = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/ai/service-assistant', {
        ...
        body: JSON.stringify({ serviceName: service.title })
      });
        ...
        <div className="flex flex-col md:flex-row gap-6 relative z-10">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-500 bg-slate-100 px-3 py-1 rounded-full">{service.category}</span>
              <span className="text-sm font-medium text-slate-500">{service.dept}</span>
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <div className="bg-slate-50 border border-slate-100 px-4 py-3 rounded-xl flex items-center gap-3">
                ...
                <div>
                  <div className="text-xs text-slate-500 font-medium">Approximate Fees</div>
                  <div className="text-sm font-bold text-slate-900">{service.approxFees}</div>
                </div>
              </div>
              <button 
                onClick={() => navigate('/offices')}
                className="bg-slate-50 border border-slate-100 px-4 py-3 rounded-xl flex items-center gap-3 hover:bg-slate-100 transition-colors cursor-pointer text-left"
              >
                <MapPin className="w-5 h-5 text-rose-500" />
                <div>
                  <div className="text-xs text-slate-500 font-medium">Nearest Office</div>
                  <div className="text-sm font-bold text-slate-900">Locate on Map</div>
                </div>
              </button>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="glass-card p-6 md:p-8">
            <h3 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2">
              <FileText className="w-6 h-6 text-brand-500" /> Service Details
            </h3>
            
            <div className="space-y-6">
              <div>
                <h4 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-2">Purpose</h4>
                <p className="text-slate-600">{service.purpose}</p>
              </div>
              <div>
                <h4 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-2">Eligibility</h4>
                <p className="text-slate-600">{service.eligibility}</p>
              </div>
              ...
              <div className="bg-amber-50 border border-amber-100 p-4 rounded-xl flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-amber-800 text-sm">{service.importantNotes}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="glass-card p-6 md:p-8 border-t-4 border-t-purple-500">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                <Bot className="w-6 h-6 text-purple-500" /> AI Assistant
              </h3>
              {!aiGuidance && !aiLoading && (
                <button onClick={fetchAiGuidance} className="text-sm bg-purple-100 text-purple-700 px-4 py-2 rounded-lg font-medium hover:bg-purple-200 transition-colors">
                  Ask Gemini
                </button>
              )}
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="border border-slate-100 rounded-xl p-4">
                    <h4 className="text-sm font-bold text-slate-900 mb-3 flex items-center gap-2">
                      <CheckCircle className="w-4 h-4 text-emerald-500" /> Helpful Tips
                    </h4>
                  ...
                  <div className="border border-slate-100 rounded-xl p-4">
                    <h4 className="text-sm font-bold text-slate-900 mb-3 flex items-center gap-2">
                      <AlertCircle className="w-4 h-4 text-rose-500" /> Common Mistakes
                    </h4>
                    <ul className="text-sm text-slate-600 space-y-2 list-disc pl-4">
                      {aiGuidance.mistakes.map((mistake, idx) => (
            ...
            {!aiGuidance && !aiLoading && (
              <p className="text-slate-500 text-sm">Need help preparing? Ask the AI Assistant for personalized guidance, tips, and common mistakes to avoid.</p>
            )}
          </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-3">
              {service.documents.map((doc) => {
                const isFoundInVault = vaultDocs.some(vd => 
                  vd.aiSummary?.documentType?.toLowerCase().includes(doc.name.toLowerCase()) || 
                  vd.originalName.toLowerCase().includes(doc.name.toLowerCase()) ||
                  doc.name.toLowerCase().includes(vd.aiSummary?.documentType?.toLowerCase() || '')
                );

                return (
                  <div key={doc.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-xl border border-slate-100 bg-white">
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input 
                        type="checkbox" 
                        className="w-5 h-5 rounded border-slate-300 text-brand-600 focus:ring-brand-500" 
                        checked={isFoundInVault || !!checklist[doc.id]}
                        onChange={() => !isFoundInVault && toggleChecklist(doc.id)}
                        disabled={isFoundInVault}
                      />
                    ...
                    {isFoundInVault ? (
                      <span className="flex items-center gap-1 text-xs font-bold text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-100">
                        <CheckCircle className="w-3.5 h-3.5" /> Available in DigiVault
                      </span>
                    ) : (
                      <button onClick={() => navigate('/vault')} className="flex items-center gap-1 text-xs font-bold text-brand-600 bg-brand-50 hover:bg-brand-100 px-3 py-1.5 rounded-lg border border-brand-100 transition-colors">
                        Upload to Vault <ArrowLeft className="w-3.5 h-3.5 rotate-135 transform scale-x-[-1]" />
                      </button>
                    )}
                  </div>
                );
          ...
          <div className="glass-card p-6 bg-gradient-to-br from-slate-900 to-slate-800 text-white">
            ...
            <p className="text-slate-300 text-sm mb-6">Make sure you have all required documents checked off before proceeding to the official portal.</p>
            
            <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4 mb-6">
              <div className="flex items-center gap-2 text-emerald-400 text-sm font-medium mb-1">
                <CheckCircle className="w-4 h-4" /> Verified Government Portal
              </div>
            ...
            <a 
              ...
              className="w-full py-3 bg-brand-500 hover:bg-brand-600 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-colors"
            >
              Apply on Official Website <ExternalLink className="w-4 h-4" />
            </a>
            <p className="text-center text-xs text-slate-400 mt-4">Opens in a new tab. We do not collect your application data.</p>
          </div>
        </div>
      </div>
    </div>
