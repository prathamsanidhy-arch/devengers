...
const Profile = () => {
  ...
  return <div className="py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      <h1 className="text-3xl font-bold text-slate-900 mb-8">{t("Profile.my_profile")}</h1>
        ...
        <div className="space-y-8">
          ...
          y: 0
            ...
            <div className="mt-4 inline-flex items-center gap-1.5 bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full text-sm font-medium border border-emerald-200">
              <Shield className="w-4 h-4" />{t("Profile.verified_citizen")}</div>
          </motion.div>
          ...
          delay: 0.1
            ...
            <h3 className="font-bold text-slate-900 mb-4 border-b border-slate-100 pb-2">{t("Profile.activity_overview")}</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                <div className="flex items-center gap-3">
                  <div className="bg-blue-100 text-blue-600 p-2 rounded-lg"><FileText className="w-5 h-5" /></div>
                  <span className="font-medium text-slate-700">{t("Profile.total_reports")}</span>
                </div>
              ...
              <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-medium text-slate-700">{t("Profile.saved_schemes")}</span>
                </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          delay: 0.2
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2">
              <Bookmark className="w-5 h-5 text-brand-500" />{t("Profile.saved_schemes")}</h3>
            
            {savedSchemes.length === 0 ? <div className="text-center py-10 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                <Bookmark className="w-10 h-10 text-slate-300 mx-auto mb-3" />
                <p className="text-slate-500">{t("Profile.you_havent_saved_any")}</p>
                <Link to="/schemes" className="text-brand-600 font-medium hover:underline mt-2 inline-block">{t("Profile.discover_schemes")}</Link>
              </div> : <div className="space-y-4">
                {savedSchemes.map((scheme, i) => <div key={i} className="p-4 bg-white border border-slate-200 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover-lift">
                    <div>
              ...
              }} className="text-xs font-medium text-red-500 hover:text-red-600 bg-red-50 px-3 py-1.5 rounded-lg whitespace-nowrap">{t("Profile.remove")}</button>
                  </div>)}
              </div>}
          </motion.div>
          ...
          delay: 0.3
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2">
              <SettingsIcon className="w-5 h-5 text-slate-500" />{t("Profile.quick_settings")}</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex items-center justify-between p-4 border border-slate-100 rounded-xl hover:bg-slate-50 cursor-pointer">
                <div>
                  <h4 className="font-medium text-slate-900">{t("Profile.email_notifications")}</h4>
                  <p className="text-xs text-slate-500">{t("Profile.get_updates_on_complaints")}</p>
                </div>
              ...
              <div className="flex items-center justify-between p-4 border border-slate-100 rounded-xl hover:bg-slate-50 cursor-pointer">
                <div>
                  <h4 className="font-medium text-slate-900">{t("Profile.sms_alerts")}</h4>
                  <p className="text-xs text-slate-500">{t("Profile.important_govt_alerts")}</p>
                </div>
