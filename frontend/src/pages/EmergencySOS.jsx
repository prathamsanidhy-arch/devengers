...
import { Siren, Phone, ShieldAlert, Navigation, AlertTriangle, HeartPulse, Shield, MapPin, Activity } from 'lucide-react';
const EmergencySOS = () => {
  const {
    t
  } = useTranslation();
  ...
  return <div className="py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      <div className="text-center mb-10">
        <h1 className="text-3xl font-bold text-slate-900 flex items-center justify-center gap-3">
          <Siren className="w-8 h-8 text-red-500 animate-pulse" />{t("EmergencySOS.emergency_sos")}</h1>
        <p className="text-slate-500 mt-2">{t("EmergencySOS.get_immediate_help_your")}</p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-10 items-center justify-center mb-16">
        ...
        <div className="relative group cursor-pointer" onClick={handleSOS}>
          ...
          scale: 0.95
            ...
            <span className="text-sm font-medium mt-2 opacity-80 uppercase tracking-widest">{t("EmergencySOS.tap_for_help")}</span>
          </motion.div>
        </div>
        ...
        <div className="glass-card p-6 w-full max-w-md border-2 border-red-100">
          <div className="flex items-center gap-3 mb-4">
             ...
             <div>
               <h3 className="font-bold text-slate-900">{t("EmergencySOS.live_location_active")}</h3>
               <p className="text-sm text-slate-500">{t("EmergencySOS.accuracy_5_meters")}</p>
             </div>
          </div>
          <div className="bg-slate-100 rounded-xl h-32 flex items-center justify-center mb-4 overflow-hidden relative">
             ...
             <div className="relative z-10 flex flex-col items-center p-3 bg-white/80 backdrop-blur-sm rounded-lg shadow-sm border border-white">
               ...
               <p className="text-xs font-bold text-slate-800">{t("EmergencySOS.new_delhi_india")}</p>
             </div>
          </div>
          {sosActive && <motion.div initial={{
          opacity: 0,
          height: 0
        }} animate={{
          opacity: 1,
          height: 'auto'
        }} className="text-sm text-red-600 bg-red-50 p-3 rounded-lg border border-red-100 font-medium text-center">{t("EmergencySOS.alert_dispatched_to_nearest")}</motion.div>}
        </div>
      </div>

      <h2 className="text-2xl font-bold text-slate-900 mb-6 border-b border-slate-200 pb-4">{t("EmergencySOS.quick_dial_numbers")}</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {emergencyContacts.map((contact, idx) => <div key={idx} className="glass-card p-6 flex items-center justify-between hover-lift">
